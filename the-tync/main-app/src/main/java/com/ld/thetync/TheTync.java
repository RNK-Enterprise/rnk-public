...
import com.ld.thetync.core.TyncCore;
import com.ld.thetync.core.ComponentLoader;
import com.ld.thetync.core.TriggerManager;
import com.ld.thetync.core.OptimizationOrchestrator;
...
public class TheTync {
    ...
    private void initializeVortexEngines(MeterRegistry meterRegistry, Instrumentation instrumentation) {
        ...
        try {
            java.io.File engineDir = new java.io.File("src/main/java/com/ld/thetync/engines");
            if (engineDir.exists()) {
                ...
                if (files != null) {
                    for (java.io.File file : files) {
                        ...
                        try {
                            Class<?> clazz = Class.forName("com.ld.thetync.engines." + className);
    ...
    private void initializeVortexTurbos(MeterRegistry meterRegistry) {
        ...
        try {
            java.io.File turboDir = new java.io.File("src/main/java/com/ld/thetync/turbos");
            if (turboDir.exists()) {
                ...
                if (files != null) {
                    for (java.io.File file : files) {
                        String className = file.getName().replace(".java", "");
                        try {
                            Class<?> clazz = Class.forName("com.ld.thetync.turbos." + className);
