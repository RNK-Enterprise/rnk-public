# RNK Curator API — Interface Specification v0.1 (open contract)

The Curator is the curation layer of the RNK ecosystem: it turns real-world
...
open**: any party may implement a competing curator, and any curator may
...
1. **Publish.** A curator serves or distributes a *corpus bundle* — a
...
2. **Resolve.** A Tync (or Bridge operator) resolves which bundle to use:
   `curator-query` (loader pair, MC version, content classes) →
   `curator-result` (selected bundle(s), matching patterns, guidance).
...
keeps the open tier fully functional without a live curator. When a curator
...
A conformant curator implementation:
...
`npm run test:curator` validates every shipped example against its schema
...
reference curator implements this shape; a public conformance suite beyond
